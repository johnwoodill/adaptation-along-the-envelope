...
predictShare <- function(models, data, crop, se = FALSE){
  ...
  cs.share0_p <-  tobit.ey(cs0C.pred, models[[1]]$scale)
  cs.share1_p <-  tobit.ey(cs1C.pred, models[[1]]$scale)
  cs.share2_p <-  tobit.ey(cs2C.pred, models[[1]]$scale)
  cs.share3_p <-  tobit.ey(cs3C.pred, models[[1]]$scale)
  cs.share4_p <-  tobit.ey(cs4C.pred, models[[1]]$scale)
  cs.share5_p <-  tobit.ey(cs5C.pred, models[[1]]$scale)

  p.share0_p <-   tobit.ey(p0C.pred, models[[2]]$scale)
  p.share1_p <-   tobit.ey(p1C.pred, models[[2]]$scale)
  p.share2_p <-   tobit.ey(p2C.pred, models[[2]]$scale)
  p.share3_p <-   tobit.ey(p3C.pred, models[[2]]$scale)
  p.share4_p <-   tobit.ey(p4C.pred, models[[2]]$scale)
  p.share5_p <-   tobit.ey(p5C.pred, models[[2]]$scale)

  diff.share0_p <- tobit.ey(diff0C.pred, models[[3]]$scale)
  diff.share1_p <- tobit.ey(diff1C.pred, models[[3]]$scale)
  diff.share2_p <- tobit.ey(diff2C.pred, models[[3]]$scale)
  diff.share3_p <- tobit.ey(diff3C.pred, models[[3]]$scale)
  diff.share4_p <- tobit.ey(diff4C.pred, models[[3]]$scale)
  diff.share5_p <- tobit.ey(diff5C.pred, models[[3]]$scale)
  ...
  pred.proportions <- list(cs.prop = data.frame(temp = rep(c(0,1,2,3,4,5),each = length(cs.share0_a)),
                                            prop = c(cs.share0_p, cs.share1_p, cs.share2_p, cs.share3_p, cs.share4_p, cs.share5_p),
                                            crop = crop),
                      p.prop = data.frame(temp = rep(c(0,1,2,3,4,5),each = length(p.share0_p)),
                                            prop = c(p.share0_p, p.share1_p, p.share2_p, p.share3_p, p.share4_p, p.share5_p),
                                            crop = crop),
                      diff.prop = data.frame(temp = rep(c(0,1,2,3,4,5),each = length(diff.share0_p)),
                                            prop = c(diff.share0_p, diff.share1_p, diff.share2_p, diff.share3_p, diff.share4_p, diff.share5_p),
                                            crop = crop))
  
  pred.change <- data.frame(temp = rep(c(0,1,2,3,4,5), 3),
