...
names(cs.corn.mod2$coefficients)[32:45] <- c("Degree Days (10-30C)", "Degree Days (10-30C) Squared", "Square Root Degree Days (34C)",
...
names(p.corn.mod2$coefficients)[61:65] <- c("Degree Days (10-30C)", "Degree Days (10-30C) Squared", "Square Root Degree Days (34C)", 
                                "Precipitation", "Precipitation Squared")
...
names(p.corn.mod4$coefficients)[61:65] <- c("Degree Days (10-30C)", "Degree Days (10-30C) Squared", "Square Root Degree Days (34C)", 
                                "Precipitation", "Precipitation Squared")

# Crop Choice Model
names(cc.corn.mod1$coefficients)
names(cc.corn.mod1$coefficients)[32:44] <- c("Avg. Temperature", "Avg. Temperature Squared", "Precipitation",
                                 "Precipitation Squared", "Latitude", "Income per Capita", "Population Density",
                                  "Population Density Squared", "Water Capacity", "Percent Clay", "Minimum Permeability", 
                                  "K-factor of Top Soil", "Best Soil Class")

names(cc.corn.mod2$coefficients)
names(cc.corn.mod2$coefficients)[32:45] <- c("Degree Days (10-30C)", "Degree Days (10-30C) Squared", "Square Root Degree Days (34C)",
                                     "Precipitation", "Precipitation Squared", "Latitude", "Income per Capita", "Population Density",
                                  "Population Density Squared", "Water Capacity", "Percent Clay", "Minimum Permeability", 
                                  "K-factor of Top Soil", "Best Soil Class")
...
star1 <- stargazer(list(cs.corn.mod1, p.corn.mod1, cc.corn.mod1, p.corn.mod3), align = FALSE, no.space = FALSE, style = "aer", digits = 4,
          ...
          add.lines = list(c("Weights", "Acres", "Acres", "None", "None"), c("Fixed-effect", "State", "County/Year", "State", "County/Year")))
...
# Model Table
# star3 <- stargazer(diff.corn.mod1, diff.corn.mod2, align = FALSE, no.space = FALSE, style = "aer", digits = 4,
#           omit = c("fips", "year"), omit.stat = c("ser", "f"), 
#           title = "Regression Models explaining Difference in Crop Revenue and Acres", 
#           column.labels = c("(1960-2000)", "(1960-2000)", "(1960-2000)", "(1960-2000)"),
...
#           notes.align = "l",
#           notes.append = FALSE,
#           add.lines = list(c("Fixed-effect", "County, Year", "County, Year")))
