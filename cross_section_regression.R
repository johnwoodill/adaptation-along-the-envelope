...
# Corn 
cs.corn.mod1 <- felm(ln_corn_rrev ~ dday0_10 + dday10_30  + dday30C + prec + prec_sq + lat + long + latlong| 0 | 0 | state,
                  data = cropdat, weights = cropdat$corn_w)
summary(cs.corn.mod1)

# Cotton
cs.cotton.mod1 <- felm(ln_cotton_rrev ~ dday0_10  + dday10_30 + dday30C + prec + prec_sq + lat + long + latlong | 0 | 0 | state,
                  data = cropdat, weights = cropdat$cotton_w)
summary(cs.cotton.mod1)

# Hay
cs.hay.mod1 <- felm(ln_hay_rrev ~ dday0_10 + dday10_30  + dday30C + prec + prec_sq + lat + long + latlong| 0 | 0 | state,
                  data = cropdat, weights = cropdat$hay_w)
summary(cs.hay.mod1)

# Wheat
cs.wheat.mod1 <- felm(ln_wheat_rrev ~ dday0_10 + dday10_30 + dday30C + prec + prec_sq  + lat + long + latlong| 0 | 0 | state,
                  data = cropdat, weights = cropdat$wheat_w)

summary(cs.wheat.mod1)

# Soybean
cs.soybean.mod1 <- felm(ln_soybean_rrev ~ dday0_10 + dday10_30  + dday30C + prec + prec_sq  + lat + long + latlong| 0 | 0 | state,
