...
predictRev <- function(models, data, crop, se = FALSE){
  ...
  # cs0C.pred_se <- boot.strap(exp(cs0C.pred$fit + cs0C.pred$res  ))
  # cs1C.pred_se <- boot.strap(exp(cs1C.pred$fit + cs1C.pred$res  ))
  # cs2C.pred_se <- boot.strap(exp(cs2C.pred$fit + cs2C.pred$res  ))
  # cs3C.pred_se <- boot.strap(exp(cs3C.pred$fit + cs3C.pred$res  ))
  # cs4C.pred_se <- boot.strap(exp(cs4C.pred$fit + cs4C.pred$res  ))
  # cs5C.pred_se <- boot.strap(exp(cs5C.pred$fit + cs5C.pred$res  ))
  # 
  ...
  cs.pred_rev0_a <- exp(cs0C.pred$fit + cs0C.pred$res )
  cs.pred_rev1_a <- exp(cs1C.pred$fit + cs1C.pred$res )
  cs.pred_rev2_a <- exp(cs2C.pred$fit + cs2C.pred$res )
  cs.pred_rev3_a <- exp(cs3C.pred$fit + cs3C.pred$res )
  cs.pred_rev4_a <- exp(cs4C.pred$fit + cs4C.pred$res )
  cs.pred_rev5_a <- exp(cs5C.pred$fit + cs5C.pred$res )
  ...
  # # Sum of redicted revenue
  cs.pred_rev0 <- sum(exp(cs0C.pred$fit + cs0C.pred$res ))
  cs.pred_rev1 <- sum(exp(cs1C.pred$fit + cs1C.pred$res ))
  cs.pred_rev2 <- sum(exp(cs2C.pred$fit + cs2C.pred$res ))
  cs.pred_rev3 <- sum(exp(cs3C.pred$fit + cs3C.pred$res ))
  cs.pred_rev4 <- sum(exp(cs4C.pred$fit + cs4C.pred$res ))
  cs.pred_rev5 <- sum(exp(cs5C.pred$fit + cs5C.pred$res ))
  ...
  pred.rev_a <- list(cs.acres = data.frame(temp = rep(c(0,1,2,3,4,5),each = length(cs.pred_rev0_a)),
                      ...
                      p.acres = data.frame(temp = rep(c(0,1,2,3,4,5),each = length(p.pred_rev0_a)),
                                            acres = c(p.pred_rev0_a, p.pred_rev1_a, p.pred_rev2_a, p.pred_rev3_a, p.pred_rev4_a, p.pred_rev5_a),
                                            crop = crop),
                      diff.acres = data.frame(temp = rep(c(0,1,2,3,4,5),each = length(diff.pred_rev0_a)),
                                            acres = c(diff.pred_rev0_a, diff.pred_rev1_a, diff.pred_rev2_a, diff.pred_rev3_a, diff.pred_rev4_a, diff.pred_rev5_a),
                                            crop = crop))
